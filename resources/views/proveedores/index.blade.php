...
@section('content')
    ...
    <div style="background: white; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); overflow: hidden;">
        <table id="tabla-proveedores" style="width: 100%; border-collapse: collapse; text-align: left;">
            <thead>
            ...
            <tbody>
                @forelse($proveedores as $proveedor)
                    <tr style="border-bottom: 1px solid #f1f5f9; transition: background 0.2s;" onmouseover="this.style.background='#fbfcfd'" onmouseout="this.style.background='transparent'">
                        <td style="padding: 16px 20px; font-weight: 600; color: #1e293b;">{{ $proveedor->nombre }}</td>
                        <td style="padding: 16px 20px; color: #64748b;">{{ $proveedor->email }}</td>
                        <td style="padding: 16px 20px; color: #64748b;">{{ $proveedor->telefono }}</td>
                        <td style="padding: 16px 20px;"><span style="background: #f1f5f9; color: #475569; padding: 4px 12px; border-radius: 20px; font-size: 0.8rem;">{{ $proveedor->categoria }}</span></td>
                        <td style="padding: 16px 20px; text-align: center;">
                            <div style="display: flex; gap: 8px; justify-content: center;">
                                <a href="{{ route('proveedores.edit', $proveedor->id) }}" class="btn-action btn-edit">Editar</a>
                                @if(auth()->user()->role === 'admin')
                                    <form action="{{ route('proveedores.destroy', $proveedor->id) }}" method="POST" style="display: inline;">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="btn-delete" onclick="return confirm('¿Eliminar?')">Eliminar</button>
                                    </form>
                                @endif
                            </div>
                        </td>
